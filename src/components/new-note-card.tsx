...
export function NewNoteCard({onNoteCreated}: NewNoteCardProps) {
  ...
  function handleStartRecording () {
    ...
    if (!isSpeechRecognitionAPIAvailable) {
      alert('Infelizmente seu navegador não suporta o reconhecimento de audio')
      return
    }
    ...
    speechRecognition.onresult = event => {
      const transcription =  Array.from(event.results).reduce((acc, result) => acc += result[0].transcript, '')
